...
### Added
- Session persistence: `client.getSession()` returns serializable session data; `createFromSession(session)` restores a client without re-authenticating

## [1.1.0] - 2026-02-15

### Added
- Sleep API: `client.sleep.getDailySleepData(date?, nonSleepBufferMinutes?)` and `client.sleep.getSleepStats(from, to)` with types `DailySleepData`, `SleepStats`, and related enums/schemas
